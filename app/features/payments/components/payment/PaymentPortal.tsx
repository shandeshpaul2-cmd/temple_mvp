...
export default function PaymentPortal({ items, userInfo, onBack, onSuccess, onError }: PaymentPortalProps) {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (data.isMock || !process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID || process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID === 'rzp_test_DEMO_KEY_ID') {
        ...
        setTimeout(async () => {
          try {
            await processPayment(`pay_mock_${Date.now()}`, data.orderId)
          } catch (error) {
            console.error('Development payment processing failed:', error)
        ...
        const razorpayOptions = {
          ...
          handler: async (response: any) => {
            try {
              await processPayment(response.razorpay_payment_id, data.orderId)
            } catch (error) {
  ...
  const processPayment = async (paymentId: string, orderId: string) => {
    try {
      ...
      onSuccess(data.receiptNumber, paymentId)
    } catch (error) {
